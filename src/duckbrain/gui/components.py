...
def embed_tool_report(report_path: Path, *, height: int = 1200) -> bool:
    ...
    with its own scripts render inside the app at all.
    ...
    ours. What actually bounds the risk is that these reports are generated by
    MRIQC and fMRIPrep from the user's own data on the user's own cluster, so
    treat the containment in ``core.report_embed.resolve_asset`` — which decides
    what may be *read and served* — as the control that is really doing work.
    Checked against Streamlit 1.56 on 2026-07-28; see ``TODO.md`` ``#23``.
    """
