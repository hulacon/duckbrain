"""Page 7: New to Talapas? — hand-holding for first-time cluster users.

duckbrain's other pages assume you know what a compute node is and why your
jobs charge to a PIRG. This one doesn't. It exists for users who are new to
cluster computing, the command line, or GitHub altogether (`TODO.md` #2): plain
words for the concepts the rest of the GUI takes for granted, curated links to
canonical tutorials, and — because a newcomer's PI is the one who owns most of
the answers — an explicit list of what to check with your PI before setting
anything up.

The doc twin of the resource list lives in ``QUICKSTART.md`` ("Completely new
to Talapas?"), for people reading the repo before they can launch the GUI. If
you edit the links here, edit them there too.
"""
...
st.markdown(
    """
    If words like *compute node*, *SLURM*, or *PIRG* are new to you, start
    here. Nothing on this page is specific to your study — it is the
    background the rest of duckbrain quietly assumes.
    ...
    whatever a tutorial (or this GUI) defaults to. Before setting these,
    ...
    licence forbids redistribution — see the Quickstart).
    ...
    logs. The command line you actually need is the handful of lines in
    `QUICKSTART.md`: clone, set up the environment, launch.
    """
)
