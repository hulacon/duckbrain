...
Typical usage::
    ...
    from duckbrain.core.qc_guidance import get_guidance, guidance_for_modality
...
class Reference:
    ...
    def __post_init__(self) -> None:
        if self.kind not in VALID_REF_KINDS:
            raise ValueError(
                f"Invalid reference kind {self.kind!r}; expected one of {sorted(VALID_REF_KINDS)}"
            )
...
_register(
    MeasureGuidance(
        ...
        references=(_PARKES_2018, _POWER_2012, _POWER_2014, _MRIQC_MEASURES, _NS_EXCLUDING),
    )
)
...
_register(
    MeasureGuidance(
        ...
        auto_flag=_IQR_FLAG,
        caveats=("Shares the segmentation dependency of the other tissue-specific measures."),
        references=(_MRIQC_MEASURES,),
    )
)
...
_register(
    MeasureGuidance(
        ...
        auto_flag=_IQR_FLAG,
        caveats=("Shares QI1's dependence on an unmasked, undenoised background."),
        references=(
            Reference(
                label="Mortamet et al. (2009)",
                detail="Magnetic Resonance in Medicine 62(2):365-372",
...
_register(
    MeasureGuidance(
        ...
        auto_flag=_IQR_FLAG,
        caveats=("The least specific of the three overlap measures; weigh it least heavily."),
        references=(_MRIQC_MEASURES,),
    )
)
...
PROCESS_GUIDANCE: tuple[ProcessNote, ...] = (
    ...
    ProcessNote(
        ...
        references=(_MRIQC_PAPER, _MRIQC_MEASURES, _NS_IQM_INTERPRETATION, _NS_EXCLUDING),
    ),
    ProcessNote(
        key="sentinel_values",
...
def guidance_markdown(modalities: Iterable[str] = ("bold", "T1w", "T2w", "dwi")) -> str:
    ...
    lines = [
        "# QC review guidance",
        "",
        "Generated from `src/duckbrain/core/qc_guidance.py`. Edit the",
