...
[`#13`](#13) conversion legibility (`#13.1` — but do `#19.8`/`#19.9` first) ·
...
**Measured 2026-07-30, and the measurement changes the priority: build
`#19.8` and `#19.9` first, then re-ask whether this is still worth building.**
Prompted by Ben asking whether saving a session's settings as the project default
already covers it. It does not — there is no save-as-default for the `convert`
checkbox, only for the ND policy, series types, task/run and fieldmap bindings —
but the more useful answer is what counting the actual burden turned up.

On `mmmsourcedata/sub-03/ses-01` (the beta tester whose protocol motivated this
item) **14 of 53 series arrive ticked for conversion and 12 of them are junk**
that has to be unticked by hand, every session, across 95 sessions. Only
`Resting_baseline` and its SBRef are wanted. But not one of the 12 needs a
project-level skip:

- **8 anat rows** — four `_ND`/non-`_ND` twin pairs, undropped because
  `nd_twin_bases` returns `[]` on this scanner. That is `#19.8`. Fix it and
  `[conversion] nd_duplicates` handles them, and *that* setting already has a
  save-as-project-default button.
- **4 `cmrr_diff_3shell_*_SBRef` rows** classified `fmap`. That is `#19.9`, and
  it is worse than clutter — those SBRefs get bound to bolds as fieldmap pairs.

So the per-session unticking this item exists to remove is, on the one real
dataset that motivated it, **entirely generated by two filed classifier bugs**.
A project-level skip would paper over both, and a skip is exactly the wrong
mechanism for them: it would convert a correctness bug into a per-project
workaround that hides it.

What is left of the item once those are fixed:
...
- **The `convert` checkbox does not generalize.** It is keyed on series number,
  ...
  the saved `dcm2bids_config.json`. A study that genuinely acquires a series it
  never wants — as opposed to one duckbrain misclassifies — still has to untick
  it once per session. Whether such a series exists in practice is now the open
  question, and the two examples this item used to cite were both the second
  kind, not the first.
