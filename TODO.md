...
## 5c. NORDIC versioning + distribution (investigated 2026-07-16, NOT built)
NORDIC is the **only stage with zero version provenance** — the container work
routes around it, since it has no container. `run_provenance(cfg, "nordic")`
returns `tool='nordic'` and *everything else empty*, so the NORDIC derivative's
`GeneratedBy` claims "generated by nordic" with no version, and `container-drift`
can't cover it. Nothing detects a toolbox change.

**It is also the stage most exposed to silent drift.** The toolbox is a git
checkout on a group-writable PIRG path
(`/gpfs/projects/hulacon/shared/mmmdata/code/NORDIC_Raw`, `drwxrwsr-x`): any
hulacon member can `git pull` it and silently change the denoising for every
project pointing there — no filename, path, or config value changes. Unlike a
container image (effectively immutable once pulled), a git checkout is *designed*
to be updated in place.

**Design (recommended, not built).** The checkout is the exact analogue of a
container's build provenance, and carries a real identity:
- `tool_version` ← `git describe --tags --always --dirty` → `v1.0.2-24-g0861968`
  (`--dirty` also catches a locally-edited toolbox = unreproducible results).
- build-provenance slot ← `SteenMoeller/NORDIC_Raw@0861968` (repo@ref, the same
  role as `nipreps/mriqc:24.0.2`), which would let `container-drift` generalize
  to NORDIC for free.
- Open decision: a container is *one* artifact pinning tool+runtime; NORDIC has
  **two independent axes** — toolbox code and the MATLAB runtime
  (`matlab_module = "matlab/R2024a"`). Recording MATLAB too is nearly free (it's
  already in config) but doesn't fit the two-column shape: needs a third column,
  or renaming `container_*` → artifact-neutral (a log migration, now cheap).
