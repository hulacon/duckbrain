...
<a id="29"></a>
## #29 — A Streamlit cache that has never invalidated

`gui/qc_panels.py::_load_metrics(mriqc_dir, modality, _fingerprint)` takes a
`(count, newest mtime)` fingerprint and its docstring says "``_fingerprint``
changes when the derivative does". It does — and Streamlit never looks at it.

`runtime/caching/cache_utils.py` builds the key by iterating the bound arguments
and skipping any whose name starts with an underscore ("Underscore-prefixed args
are deliberately excluded from hashing"; the branch is one `startswith("_")`
test, verified against the pinned streamlit 1.56). So the real key is
`(mriqc_dir, modality)`, and a *rerun* of MRIQC writing new metrics to the same
path serves the previous run's DataFrame until the server restarts. On a project
being dogfooded, that is the case that matters.

The convention exists for the opposite need — passing an unhashable object
Streamlit must not try to hash — and reads as "internal", which is how a cache
key ended up wearing it.

- The fix is one character: rename to `fingerprint`. `_fingerprint_of` (the
  helper) can keep its underscore; it is genuinely private.
- **Needs a test that fails first**, and the test is the point: call it, change
  a file's mtime, call again, assert the second result reflects the change.
  Nothing today would notice the cache going stale again.
- `gui/conversion_panels.py::_probe_cached` already names its parameter
  `fingerprint` for exactly this reason and says so in the docstring — copy that,
  and don't "fix" it back to match `qc_panels`.

Found 2026-08-03 while wiring `#22`, whose probe cache was about to copy the
pattern.
